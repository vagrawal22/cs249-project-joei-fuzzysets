...
class Trainer(object):
    ...
    def build(self, multiG, method='transe', bridge='CMP-linear',  dim1=200, dim2=200, batch_sizeK1=1024, batch_sizeK2=1024, 
    ...
    def train(self, epochs=20, save_every_epoch=10, lr=0.001, a1=0.1, a2=0.05, m1=0.5, m2=1.0, AM_fold=1, half_loss_per_epoch=-1):
        ...
        print("MTransE saved in file: %s. Multi-graph saved in file: %s" % (this_save_path, self.multiG_save_path))
        print("Done")
    def load_model(self, save_path='this-model.ckpt'):
        self.sess = tf.Session()
        self.tf_parts._saver.restore(self.sess, save_path)
...
# def load_tfparts(multiG, method='transe', bridge='CMP-linear', dim1=300, dim2=100, batch_sizeK1=1024, batch_sizeK=1024, batch_sizeA=64, save_path='this-model.ckpt', L1=False):
#     tf_parts = model.TFParts(
#         num_rels1=multiG.KG1.num_rels(),
#         num_ents1=multiG.KG1.num_ents(),
#         num_rels2=multiG.KG2.num_rels(),
#         num_ents2=multiG.KG2.num_ents(),
#         method=method,
...
#     # Use tf.train.Saver to load the model with the option to ignore missing variables
#     sess = tf.Session()
...
#         print("Attempting to load model ignoring missing variables...")
#         # Load the model ignoring missing variables
#         var_list = [var for var in tf.global_variables() if sess.run(tf.is_variable_initialized(var))]
...
def load_tfparts(multiG, method='transe', bridge='CMP-linear', dim1=300, dim2=100, batch_sizeK1=1024, batch_sizeK=1024, batch_sizeA=64, save_path='this-model.ckpt', L1=False):
    ...
    # Get the variable names from the checkpoint
    checkpoint_vars = tf.train.list_variables(save_path)

    # Create a dictionary to map variable names to their shapes
    checkpoint_var_shapes = {name: shape for name, shape in checkpoint_vars}

    # Create a new Saver that will ignore variables with shape mismatches
    var_list = []
    for var in tf.global_variables():
        var_name = var.name.split(':')[0]
        if var_name in checkpoint_var_shapes:
            if var.shape.as_list() == checkpoint_var_shapes[var_name]:
                var_list.append(var)
